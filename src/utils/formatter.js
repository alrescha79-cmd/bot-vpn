...
exports.formatResellerStats = formatResellerStats;
exports.formatTopResellers = formatTopResellers;
exports.formatUserMention = formatUserMention;
exports.formatRoleLabel = formatRoleLabel;
exports.formatServerList = formatServerList;
 ...
 * @returns {string}
 */
function formatServerInfo(server, flagEmoji = '🌐') {
...
function formatInvoice(data) {
    const { mention, actionLabel, label, serverNama, ipLimit, username, type, hargaFinal, durasiHari, komisi, isReseller, waktuSekarang } = data;
    return `
━━━━━━━━━━━━━━━━━━━━━━━        
 ...
 * @returns {string}
 */
function formatResellerStats(data) {
    const { level, totalAkun, totalKomisi, recentSales } = data;
    const list = recentSales.map((r, i) => `🔹 ${r.akun_type.toUpperCase()} - ${r.username} (+Rp${r.komisi}) 🕒 ${r.created_at}`).join('\n') || '_Belum ada transaksi_';
 ...
 * @returns {string}
 */
function formatTopResellers(resellers) {
    const medals = ['🥇', '🥈', '🥉'];
 ...
 * @returns {string}
 */
function formatUserMention(ctx) {
 ...
 * @returns {string}
 */
function formatRoleLabel(role, resellerLevel = 'silver') {
    if (role === 'admin')
        return '👑 Admin';
    if (role === 'reseller')
        return `🏆 Reseller (${resellerLevel.toUpperCase()})`;
    return 'User';
}
/**
